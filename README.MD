...
```
<!-- dobounce  timeout 下一次把上一次给clear了 防止每次一写就直接发送请求数据 -->
<!-- 可以设置一个test user -->
<!-- 还可以给testuser设置中间件 不让他update read only -->
```js
Procfile
...
<!-- cookie可以实现记住用户名密码 也可以实现 7天免登录这种 token就是检查请求的header这种 -->


<!-- 因为cookie把 token的都删了 所以每次刷新页面会出问题 直接写一个前端的getcurrentuser 发送currentuser请求到server 这样就有了req  有了请求就知道请求的user了 因为以前都是由token的 加一个route 是 getcurrentuser     就是获取 req.user 注意 直接在context里面使用getcurrentuser  每次刷新context useeffect[]直接 获取到currentuser -->

<!-- logout的时候 直接改cookie时间 -->